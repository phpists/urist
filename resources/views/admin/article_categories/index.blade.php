                    ...
                    <div class="tab-content">
                        ...
                        <div class="tab-pane fade show" role="tabpanel" id="tree_tab">
                            <div id="jstree_container">
                                <ul>
                                    @foreach($article_categories->whereNull('parent_id') as $article_category)
                                        <li id="node_{{$article_category->id}}">{{$article_category->name}}</li>
                                    @endforeach
                                </ul>
                            </div>
                        </div>
...
@section('js_after')
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            jstreeEl.on('select_node.jstree', function (ev, node) {
                ...
                $.ajax({
                    ...
                    data: {
                        id: node.node.id.split('_')[1]
                    },
                    success: function (resp) {
                        resp.forEach((el) => {
                            jstreeEl.jstree('create_node', $('#' + node.node.id), {
                                "id": "node_" + el.id,
                                "text": el.name
                            }, 'last', false, false)
                        })
            ...
            jstreeEl.on('move_node.jstree', function (node, parent) {
                console.log(node);
                console.log(parent);
            })
        })
