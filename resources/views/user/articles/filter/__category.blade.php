...
<div class="accordion__panel sub-category">
    <div class="accordion__header" id="accordion-header-{{ $category->id }}" style="padding-left: {{ $padding + 10 }}">
        ...
        <button type="button" class="accordion__trigger" aria-expanded="{{ $isMustBeExpanded ? 'true' : 'false' }}" aria-controls="accordion-content-{{ $category->id }}">
    ...
    <div class="accordion__content" id="accordion-content-{{ $category->id }}" role="region" aria-labelledby="accordion-header-{{ $category->id }}" aria-hidden="{{ $isMustBeExpanded ? 'false' : 'true' }}">
        @if($category->children->isNotEmpty())
